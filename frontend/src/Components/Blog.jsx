...
const handleAddComment = async (blogId) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      {/* Navbar */}
      <nav className="bg-slate-950 border-b border-emerald-500/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              <Link to="/community-post" className="text-white hover:text-emerald-400 transition-colors">Community Post</Link>
              <button
                onClick={() => setIsModalOpen(true)}
                className="inline-flex items-center gap-2 bg-emerald-500 text-black px-6 py-2 rounded-lg hover:bg-emerald-600 transition-colors font-semibold text-lg"
              >
                <Upload size={20} />
                Start Writing
              </button>
          ...
          {isNavOpen && (
            <div className="md:hidden pb-4 space-y-3">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-emerald-500 text-black px-6 py-2 rounded-lg hover:bg-emerald-600 transition-colors font-semibold"
              >
                <Upload size={20} />
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="border-b border-emerald-500/20 p-6 flex justify-between items-center sticky top-0 bg-gray-900">
              <h2 className="text-2xl font-bold text-emerald-400">{isEditing ? "Edit Blog" : "Upload Blog"}</h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-800 text-white border border-gray-700 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-500 transition-colors placeholder-gray-500"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="6"
                  className="w-full bg-gray-800 text-white border border-gray-700 rounded-lg px-4 py-3 focus:outline-none focus:border-emerald-500 transition-colors placeholder-gray-500 resize-none"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  required
                  className="flex-1 bg-gray-800 text-white border border-gray-700 rounded-lg px-4 py-3 file:bg-emerald-500 file:text-black file:border-0 file:rounded file:px-3 file:py-1 file:cursor-pointer file:font-semibold hover:file:bg-emerald-600 transition-colors"
                />
                {thumbnailPreview && (
                  <img
                    src={thumbnailPreview || "/placeholder.svg"}
                    alt="Thumbnail preview"
                    className="mt-3 w-full h-48 object-cover rounded-lg border border-emerald-500/30"
                  />
                )}
              </div>
              <div className="flex gap-3 pt-4">
                <button
                  type="submit"
                  className="flex-1 bg-emerald-500 text-black font-semibold py-3 rounded-lg hover:bg-emerald-600 transition-colors flex items-center justify-center gap-2"
                >
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex justify-center gap-4">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-semibold ${filterType === "user" ? "bg-emerald-500 text-black" : "bg-gray-700 text-white"}`}
            >
              User
            </button>
            <button
              onClick={() => setFilterType("trainer")}
              className={`px-4 py-2 rounded-lg font-semibold ${filterType === "trainer" ? "bg-emerald-500 text-black" : "bg-gray-700 text-white"}`}
            >
              Trainer
            </button>
          </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-4 mb-4 border-t border-gray-700 pt-4">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-800 text-gray-400 hover:text-emerald-400 hover:bg-gray-700 transition-colors"
                    >
    ...
    {/* Share */}
                  <button
                    ...
                    className="p-2 rounded-lg bg-gray-800 text-gray-400 hover:text-emerald-400 hover:bg-gray-700"
                    title="Share"
                  >
                    <Share2 size={16} />
                  </button>
                    ...
                    <div className="border-t border-gray-700 pt-4 space-y-3">
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-2">
                        {commentStates[blog.id]?.comments?.map((comment, idx) => (
                          <div key={idx} className="bg-gray-800 p-3 rounded-lg">
                            <p className="text-emerald-400 font-semibold text-sm">{comment.userName}</p>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-gray-800 text-white border border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 transition-colors"
                        />
                        <button
                          onClick={() => handleAddComment(blog.id)}
                          className="bg-emerald-500 text-black px-3 py-2 rounded-lg hover:bg-emerald-600 transition-colors font-semibold text-sm"
                        >
                          Post
                        </button>
                      </div>
